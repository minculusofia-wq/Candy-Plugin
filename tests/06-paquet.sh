...
# Monitor lance aussi des commandes : un commit de clôture passé par lui doit
# consommer le témoin comme un autre (relecture xhigh).
apres = [h["command"] for g in json.load(open("hooks/hooks.json"))["hooks"].get("PostToolUse", [])
         if {"Bash", "Monitor"} <= set(re.split(r"[|,]", g.get("matcher", ""))) for h in g["hooks"]]
